  ...
  const handleScan = async (file: File) => {
    ...
    try {
      ...
      if (err.message?.includes('429') || err.message?.includes('Quota exceeded') || err.message?.includes('RESOURCE_EXHAUSTED')) {
        setError("Batas penggunaan (Quota) API Gemini telah tercapai. Mohon tunggu sekitar 1 menit sebelum mencoba lagi. Ini adalah batasan dari layanan gratis Google Gemini.");
